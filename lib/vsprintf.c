 ...
 * @qualifier: qualifier of a number (long, length_t, ...)
 */
static
...
qualifier:
	...
	if (spec->qualifier == 'L')
		...
		spec->type = FORMAT_TYPE_LENGTH_T;
	} else if (spec->qualifier == 'H') {
		if (spec->flags & SIGN)
 ...
 *   case.
 * %pV recursive vsnprintf" via va_format_t.
 * %c honors precision field to mean count (which can be 0).
 * %n is ignored
 *
...
int vsnprintf(char *buf, length_t size, const char *fmt, va_list args)
{
	...
	/* Reject out-of-range values.  Large positive sizes are
	   used for unknown buffer sizes. */
	BUG_ON((int) size < 0, "%s < 0", size, 0);
	...
	while (*fmt) {
		...
		case FORMAT_TYPE_CHAR: {
			...
			/*
			 * We can use the precision to
			 * control how many of this character
			 * is printed. Useful for padding
			 * out other things.
			 */
			int count = spec.precision >= 0 ?
				spec.precision : 1;
		...
		case FORMAT_TYPE_NRCHARS: {
			...
			if (qualifier == 'l') {
				...
				*ip = (str - buf);
				...
				length_t *ip = va_arg(args, length_t *);
				*ip = (str - buf);
			} else {
		...
		default:
			...
			case FORMAT_TYPE_LONG:
				...
				break;
			case FORMAT_TYPE_LENGTH_T:
				num = va_arg(args, length_t);
				break;
 ...
 */
int vscnprintf(char *buf, length_t size, const char *fmt, va_list args)
{
	int i;
 ...
 * See the vsnprintf() documentation for format string extensions over C99.
 */
int snprintf(char *buf, length_t size, const char *fmt, ...)
{
	va_list args;
	int i;
...
int scnprintf(char *buf, length_t size, const char *fmt, ...)
{
	va_list args;
	int i;
 ...
 */
int vbin_printf(uint32_t *bin_buf, length_t size, const char *fmt, va_list args)
{
	struct printf_spec spec = {0};
	char *str, *end;
 ...
 * string is truncated.
 */
int bstr_printf(char *buf, length_t size, const char *fmt, const uint32_t *bin_buf)
{
	struct printf_spec spec = {0};
	char *str, *end;
 ...
 * into @bin_buf.
 */
int bprintf(uint32_t *bin_buf, length_t size, const char *fmt, ...)
{
	va_list args;
	int ret;
...
int vsscanf(const char *buf, const char *fmt, va_list args)
{
	...
	while (*fmt && *str) {
		...
		case 'z':
		{
			length_t *s = (length_t *)va_arg(args, length_t *);
			*s = (length_t)simple_strtoul(str, &next, base);
		}
		break;
		default:
			if (is_sign) {
